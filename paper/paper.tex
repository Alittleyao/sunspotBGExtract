...
假设提取到的原始数据的集合为$D = \{\vb*{x}_1, \vb*{x}_2, \ldots, \vb*{x}_n\}$，并且每个$\vb*{x}_j$为$d$维的向量，在给定分类组数$k(k\leq n)$的条件下，将原始数据划分成个$k$个子集$\vb*{S} = \{S_1, S_2, \ldots, S_k\}$，$\vb*{\mu}_i$为样本$S_i$的中心点。K-means聚类的目的就是找到一个划分$\vb*{S}$，使其类内平方和最小，即每个点到其所属类的中心点的距离的平方和最小。用数学表达式表达，即求解下述最优化问题：
\begin{equation}
...
\item 对剩余的每一个点$\vb*{x}_j$计算其到中心点的距离，并把它归到距离最近的中心点的类，计算准则为：
  ...
  \end{equation}
  其中，$S_i^{(t)}$表示第$t$此迭代后属于第$i$类的点的集合，$\vb*{\mu}_i^{(t)}$为第$t$次迭代前的中心点。
