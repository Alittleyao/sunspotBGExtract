...
\textbf{摘要: }针对太阳黑子手绘图中印刷与手绘部分的分割问题，提出了一种基于Lab色彩空间的像素聚类方法。该方法首先计算手绘图在Lab色彩空间中的$a$、$b$分量，利用这两个分量对像素点进行K-means聚类。结果表明此方法准确的分割了手绘图中的印刷与手绘部分。为提高批量分割的计算速度，对所有的手绘图进行抽样，对样本总体利用上述方法分割后，直接利用样本的分割准则对其他的手绘图进行分割。分析表明，此方法在极大地提高计算速度的同时保证了分割的准确性。
